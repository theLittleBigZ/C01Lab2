...
app.get("/getAllNotes/", express.json(), async (req, res) => {
  try {
    ...
    jwt.verify(token, "secret-key", async (err, decoded) => {
      ...
      //getting all notes with username then converting to array
      const collection = db.collection(COLLECTIONS.notes);
  ...
  try {
    ...
    jwt.verify(token, "secret-key", async (err, decoded) => {
      ...
      if (result.deletedCount === 1) {
        ...
        res.status(404).json({ error: "Note not found or does not belong to the user." });
      }
    });
  } catch (error) {
    res.status(500).json({ error: error.message });
...
app.patch("/editNote/:noteId", express.json(), async (req, res) => {
  try {
    ...
    jwt.verify(token, "secret-key", async (err, decoded) => {
      ...
      //getting edited title and content
      const { title, content } = req.body;
      const updateFields = {};
